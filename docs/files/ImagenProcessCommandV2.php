...
class ImagenProcessCommand extends Command
{
    ...
    private function stepProcess(): void
    {
        ...
        $editOptions = new ImagenEditOptions(
            ...
            photographyType: $this->job->photography_type
                ? ImagenPhotographyType::from(strtoupper($this->job->photography_type))
                : null
        );
    ...
    private function stepExport(): void
    {
        ...
        spin(
            callback: function() {
                ...
                sleep(5); // Give API time to process export
            },
            message: 'Exporting...'
        );
    ...
    private function stepDownload(): void
    {
        ...
        $this->job->markComplete($downloadResult->succeeded());
        note("✓ Downloaded {$downloadResult->succeeded()} images to {$editedDir}");
        $this->newLine();
    }
    ...
    private function getEditOptions(): ImagenEditOptions
    {
        ...
        if (!confirm('Use default edit options?', default: true)) {
            ...
            return new ImagenEditOptions(
                ...
                hdrMerge: $hdr,
                photographyType: $type ? ImagenPhotographyType::from(strtoupper($type)) : null
            );
        }
